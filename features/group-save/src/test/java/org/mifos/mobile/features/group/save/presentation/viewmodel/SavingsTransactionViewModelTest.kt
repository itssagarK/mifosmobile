...
class SavingsTransactionViewModelTest {
    ...
    private val getTransactionsNonPaged: GetSavingsTransactionsNonPagedUseCase = mockk()
    ...
    fun setUp() {
        ...
        every { useCases.getTransactionsNonPaged } returns getTransactionsNonPaged
        ...
        every { getTransactions(any(), any(), any(), any(), any(), any()) } returns flowOf(androidx.paging.PagingData.empty())
    ...
    @Test
    fun `initial summary state should be Success with transactions when usecase has data`() = runTest {
        // Given
    ...
    @Test
    fun `refresh failure should show error summary state when transactions are empty`() = runTest {
        // Given
        every { getTransactionsNonPaged(any(), any(), any(), any(), any(), any()) } returns flowOf(emptyList())
